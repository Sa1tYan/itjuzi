...
from .settings import USER_AGENT_LIST, COOKIE_LIST_DETAIL
from .settings import COOKIE_LIST
from .cookie_test import GetCookie
...
class CookieMiddleware(object):
    ...
    def __init__(self):
        self.count = 1
        self._cookie_list = COOKIE_LIST_DETAIL
        self.get_cookie = GetCookie()

    def process_request(self, request, spider):
        self.count += 1
        if self.count >= 50:
            self._cookie_list.pop(0)
            new_cookie = self.get_cookie.login()
            print("change_cookie_ing")
            time.sleep(2)
            self._cookie_list.append(new_cookie)
            self.count = 1
        cookie = random.choice(self._cookie_list)
        request.cookies = cookie
...
# class RetryProxyMiddleware(RetryMiddleware):
    def process_response(self, request, response, spider):
        ...
        if request.meta.get('dont_retry', False):
            return response
        if response.status in self.retry_http_codes or len(response.text) < 250:
